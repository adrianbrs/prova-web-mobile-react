...
export class ApiResource<T = any> {
    ...
    get<R = T | T[]>(url: string | number = '', config?: AxiosRequestConfig) {
        return this.axios.get(url?.toString() || '', config).then(res => res.data as R);
    }

    post<R = T>(url: string | number = '', data: any, config?: AxiosRequestConfig) {
        return this.axios.post(url?.toString() || '', data, config).then(res => res.data as R);
    }
    ...
    createOne<D = T>(data: D, config?: AxiosRequestConfig) {
        return this.post<D>('', data, config);
    }

    updateOne<D = T>(data: D, config?: AxiosRequestConfig) {
        return this.post<D>('', data, config);
    }
}